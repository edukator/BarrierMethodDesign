function [Xf, Xp, resampling_counter, W_history] = girsanov_with_plot_standardizing( ...
...
for obs_idx = 1:nt
    ...
    meas_term = -(1/(2*s2z)) .* sum((obs_z - H * Xnew).^2, 1);  % 1xN
                                % 1xN
    meas_term=(meas_term - mean(meas_term)) ./ ...
         (std(meas_term) + eps);
    
    girs_term = (adjusted_weights_arr - mean(adjusted_weights_arr)) ./ ...
         (std(adjusted_weights_arr) + eps);

    llk = meas_term+girs_term;

    % --- Compute weights as usual ---
    lw = llk - max(llk);   % subtract max for stability
