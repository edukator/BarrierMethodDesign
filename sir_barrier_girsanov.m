...
function [Xf, Xp,resampling_counter,W_history] = sir_barrier_girsanov(F,sx,sz,h,NT,n_obs,z,H,X0,ness_thr,barrier_params)
...
for obs_idx=1:nt
   ...
   for inner_idx=1:n_obs
       ...
       % fprintf("finer_idx  % d, Xp stored at   %d \n", finer_idx_counter,(obs_idx-1)*n_obs+inner_idx+1);
        ...
        stochastic_integral=(1/sx)*stochastic_integral;
   ...
   % prediction
       ...
       llk = -(1/(2*s2z)) .* sum(( obs_z - H*Xnew ).^2)+adjusted_weights_arr;  % log-likelihood
       lw=lw+llk;
       lw = lw - max(lw);
