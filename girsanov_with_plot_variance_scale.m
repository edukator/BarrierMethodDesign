function [Xf, Xp, resampling_counter, W_history] = girsanov_with_plot_variance_scale( ...
...
for obs_idx = 1:nt
    ...
    terms_2N = [meas_term; girs_term];
    %% a try for scaling
    % Compute variance proxies
    var_meas = var(meas_term);             % across particles
    var_adj  = var(adjusted_weights_arr);  % across particles

    % Scale adjusted term to match variance
    scale = sqrt(var_meas / (var_adj + eps));


    % Build log-likelihood with your reduce_factor
    reduce_factor = scale;
    llk = meas_term + reduce_factor * girs_term;   % 1xN
    girs_term =reduce_factor * adjusted_weights_arr;  
