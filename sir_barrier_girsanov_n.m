...
function [Xf, Xp,resampling_counter,W_history] = sir_barrier_girsanov_n(F,sx,sz,h,NT,n_obs,z,H,X0,ness_thr,barrier_params)
...
for obs_idx=1:nt
   ...
   for inner_idx=1:n_obs
       ...
       % fprintf("finer_idx  % d, Xp stored at   %d \n", finer_idx_counter,(obs_idx-1)*n_obs+inner_idx+1);
        %% girsanov weighting:
         deterministic_integral=deterministic_integral+vecnorm(mu*gradL).^2;
        stochastic_integral=stochastic_integral+mu*sum(gradL.*dWx);

   end 
   
        deterministic_integral=-0.5/s2x*deterministic_integral*h;
        stochastic_integral=(1/s2x)*stochastic_integral;%  Wx include sx
        adjusted_weights_arr=deterministic_integral+stochastic_integral;
   ...
   % prediction
       ...
       % weights
       reduce_factor=0.4;
       llk = -(1/(2*s2z)) .* sum(( obs_z - H*Xnew ).^2)+reduce_factor*adjusted_weights_arr;  % log-likelihood
       lw=lw+llk;
       lw = lw - max(lw);
